...
from lib.detector.BSL import BSL
...
class bsl(TwoStage):
    ...
    def modify_commandline_options(parser, is_test):
        ...
        model_name += 'cat' if len(cfg.in_level) > 1 else ""
    ...
    def set_input(self, input):
        ...
        # if 'vis_inp' in self.visual_names:
        # self.vis_inp = vutils.make_grid(util.recon_image(input['img']), nrow=8, padding=2,
        #                                 normalize=True, scale_each=True, pad_value=1)
        # visualization
        # util.plot_gt_on_img(images, targets, write_path=
        # "/home/caffe/code/deep-person-search/cache/img_with_gt_box/gt%d.png" % np.random.choice(list(range(10)), 1))

        # images, targets = util.background_augmentation(images, targets, cfg.benchmark, pid2BoxPath=None)
